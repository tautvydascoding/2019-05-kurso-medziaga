...
function createPatient( $vardas, $pavarde, $doctorId) {
    $vardasUzkoduotas = mysqli_real_escape_string (getPrisijungtimas(), $vardas );
    $pavardeUzkoduotas = mysqli_real_escape_string (getPrisijungtimas(), $pavarde );
    $doctorIdUzkoduotas = mysqli_real_escape_string (getPrisijungtimas(), $doctorId );

    $manoSQL = "INSERT INTO  patients
                    VALUES( null, '$vardasUzkoduotas', '$pavardeUzkoduotas', '$doctorIdUzkoduotas') ";
    $rezultatai = mysqli_query(getPrisijungtimas(),  $manoSQL); // print_r(    $rezultataiOBJ );  // test
    if ( !$rezultatai) {
        echo "ERROR: nepavyko sukurti gydytojo. SQL klaida:" . mysqli_error(getPrisijungtimas());
    } else {
...
function    updatePatient($nr, $vardas, $pavarde, $doctorId) {
    $nrUzkoduotas = mysqli_real_escape_string (getPrisijungtimas(), $nr );
    $vardasUzkoduotas = mysqli_real_escape_string (getPrisijungtimas(), $vardas );
    $pavardeUzkoduotas = mysqli_real_escape_string (getPrisijungtimas(), $pavarde );
    $doctorIdUzkoduotas = mysqli_real_escape_string (getPrisijungtimas(), $doctorId );
...
function getPatients($kiekPatients = 99999) {
    $manoSQL = "SELECT * FROM patients LIMIT  $kiekPatients   ";
    $rezultatai = mysqli_query(getPrisijungtimas(),  $manoSQL); // print_r(    $rezultataiOBJ );  // test

    if ( !$rezultatai) {
        echo "ERROR: Neradome pacientu. SQL klaida:" . mysqli_error(getPrisijungtimas());
        return null;
    } else {
...
// }
// $patientsOBJ = getPatients(4);
// print_r($patientsOBJ); //test
// ar radome pacientu
// if (mysqli_num_rows($patientsOBJ) > 0) {
//     $vienasPatientArray = mysqli_fetch_assoc($patientsOBJ);     // mysqli_fetch_assoc - paima viena eilue is OBJ ir pavercia i array
//     while ($vienasPatientArray == true) {
//
//         echo "<h2>". $vienasPatientArray['name'] ."</h2>";
//         $vienasPatientArray = mysqli_fetch_assoc($patientsOBJ);     // mysqli_fetch_assoc - paima viena (SEKANCIA) eilue is OBJ ir pavercia i array
//     }
// } else {
//     echo "NEI NIEVO PACIENTO NERADOME <br />";
// }
